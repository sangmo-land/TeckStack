...
import { Star, Users, Clock, Heart, ShoppingCart, CheckCircle, ChevronDown, BookOpen, Award, Globe, Video, FileText, HelpCircle, Play, Code, Copy, Check, ExternalLink, X, ArrowLeft, ArrowRight, Twitter, Linkedin } from 'lucide-react';
        ...
        <div className="bg-slate-900/80 rounded-lg border border-slate-700/50 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 bg-slate-800/50 border-b border-slate-700/50">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-slate-300">{snippet.title}</span>
                    <span className={`px-2 py-0.5 text-xs rounded-full bg-gradient-to-r ${getLanguageColor(snippet.language)} text-white font-medium`}>
...
function ChapterItem({ chapter, level = 1, index = 0, onOpenChapter, showCodeSnippets = true }) {
    ...
    const getLevelStyles = () => {
        switch(level) {
            case 1:
                return {
                    container: "bg-slate-900/50 border border-slate-700/50 rounded-xl overflow-hidden hover:border-purple-500/30",
            ...
            default:
                return {
                    ...
                    summary: `px-${4 + level} py-3 cursor-pointer hover:bg-slate-700/10`,
    ...
    if (!hasChildren) {
        return (
            <div className="bg-slate-800/20">
                ...
                >
                    <div className={`${styles.badge} flex items-center justify-center`}>
                        {chapter.full_number}
                    </div>
                ...
                {hasSnippets && (
                    <div className="px-5 py-4 space-y-3 bg-slate-950/30">
                        {showCodeSnippets && chapter.code_snippets.map((snippet) => (
    ...
    return (
        <details className={`group ${styles.container} transition-all duration-300`}>
            <summary className={`${styles.summary} transition-colors flex items-center justify-between list-none [&::-webkit-details-marker]:hidden`}>
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">
                            ...
                            {chapter.duration_minutes && ` • ${chapter.duration_minutes} min`}
                            {hasSnippets && ` • ${chapter.code_snippets.length} code snippets`}
                        </p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
...
export default function CourseShow({ course, isEnrolled, isWishlisted, userReview, reviews }) {
    ...
    const currentChapterIndex = selectedChapter
        ...
        : -1;
    const previousChapter = currentChapterIndex > 0 ? flatChapters[currentChapterIndex - 1] : null;
    const nextChapter =
    ...
    useEffect(() => {
        ...
        const target = flatChapters.find((c) => c.id === (Number.isNaN(numericId) ? chapterIdParam : numericId));
        if (target) {
    ...
    const handleWishlist = () => {
        ...
        router.post(`/courses/${course.slug}/wishlist`, {}, {
            preserveScroll: true,
            onSuccess: () => {
                // Optionally show a success message
            },
    ...
    const handleReviewSubmit = (e) => {
        ...
        router.post(`/courses/${course.slug}/reviews`, { 
            ...reviewForm, 
            update: !!userReview 
        }, {
            preserveScroll: true,
    ...
    const getInstructorImageUrl = () => {
        const raw = course?.instructor?.instructorProfile?.avatar_url
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
                ...
                <section 
                    ...
                    style={course.cover_image ? {
                        backgroundImage: `url('${course.cover_image && (course.cover_image.startsWith('http') || course.cover_image.startsWith('/storage/')) ? course.cover_image : `/storage/${course.cover_image}`}')`,
                    ...
                    <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center">
                        <div className="flex-1 py-12">
                            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500/20 backdrop-blur-sm border border-blue-400/30 rounded-full mb-4">
                                ...
                                <span className="text-blue-200 text-sm font-semibold uppercase tracking-wide">{course.category}</span>
                            </div>
                                    ...
                                    <div className="flex items-center gap-1">
                                        {[...Array(5)].map((_, i) => (
                                            <Star 
                                                key={i} 
                                                size={18} 
                                                className={i < Math.floor(parseFloat(course.rating) || 0) ? "fill-yellow-400 text-yellow-400" : "fill-slate-600 text-slate-600"} 
                                            />
                                        ))}
                                    </div>
                                    <span className="font-semibold">{parseFloat(course.rating || 0).toFixed(1)}</span>
                                    <span className="text-slate-300">({reviews.total} reviews)</span>
                                </div>
                                
                                <div className="flex items-center gap-2 bg-slate-800/50 backdrop-blur-sm px-4 py-2 rounded-lg border border-slate-700/50">
                                    <Users size={18} className="text-blue-400" />
                                    <span className="font-semibold">{course.total_students.toLocaleString()}</span>
                                    <span className="text-slate-300">students enrolled</span>
                                </div>
                                
                                <div className="flex items-center gap-2 bg-slate-800/50 backdrop-blur-sm px-4 py-2 rounded-lg border border-slate-700/50">
                                    <Globe size={18} className="text-green-400" />
                                    <span className="font-semibold capitalize">{course.language}</span>
                                </div>
                            </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <section className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
                            ...
                            <p className="text-slate-300 text-lg leading-relaxed">{course.overview}</p>
                        </section>
                            ...
                            <section className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {course.learning_outcomes.map((outcome, idx) => (
                                        ...
                                        >
                                            <CheckCircle className="text-green-400 flex-shrink-0 group-hover:scale-110 transition-transform" size={20} />
                                            <span className="text-slate-300 group-hover:text-slate-100 transition-colors">{outcome}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
                            ...
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="text-sm text-slate-400">
                                        {course.root_chapters?.length || 0} chapters
                                    </div>
                                </div>
                            </div>
                            <div className="space-y-3">
                                {course.root_chapters?.map((chapter, idx) => (
                                    <ChapterItem 
                                        key={chapter.id} 
                                        chapter={chapter} 
                                        level={1}
                        ...
                        <section className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
                            <div className="flex items-center gap-3 mb-8">
                                ...
                                <h2 className="text-3xl font-bold text-white">Your Instructor</h2>
                            </div>
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 mb-4 text-slate-300 text-sm">
                                        {course.instructor.role && (
                                            <span className="inline-flex items-center gap-2 px-2 py-1 bg-slate-900/50 border border-slate-700/50 rounded">
                                                ...
                                                {course.instructor.role.charAt(0).toUpperCase() + course.instructor.role.slice(1)}
                                            </span>
                                        )}
                                        ...
                                        {course.instructor.created_at && (
                                            <span className="inline-flex items-center gap-2 px-2 py-1 bg-slate-900/50 border border-slate-700/50 rounded">
                                                ...
                                                Member since {new Date(course.instructor.created_at).getFullYear()}
                                            </span>
                                        )}
                                    </div>
                                    {course.instructor.instructorProfile && (
                                        <>
                                            <div className="flex flex-wrap gap-2 mb-4">
                                                {course.instructor.instructorProfile.expertise?.map((exp, idx) => (
                                                    <span key={idx} className="px-3 py-1 bg-blue-500/20 border border-blue-400/30 text-blue-300 text-sm rounded-full">
                                                        {exp}
                                                    </span>
                                                ))}
                                            </div>
                                            <div className="flex items-center gap-2 mb-2">
                                                {course.instructor.instructorProfile.is_verified && (
                                            ...
                                            <p className="text-slate-300 mb-6 leading-relaxed">{course.instructor.instructorProfile.bio}</p>
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4">
                                                    <p className="text-sm text-slate-400 mb-1">Total Students</p>
                                                    <p className="text-2xl font-bold text-white">{course.instructor.instructorProfile.total_students?.toLocaleString() || 0}</p>
                                                </div>
                                                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4">
                                                    <p className="text-sm text-slate-400 mb-1">Courses</p>
                                                    <p className="text-2xl font-bold text-white">{course.instructor.instructorProfile.total_courses || 0}</p>
                                                </div>
                                                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4">
                                                    <p className="text-sm text-slate-400 mb-1">Rating</p>
                                                    <div className="flex items-center gap-2">
                                                        <Star size={18} className="fill-yellow-400 text-yellow-400" />
                                                        <p className="text-2xl font-bold text-white">{course.instructor.instructorProfile.average_rating || parseFloat(course.rating || 0).toFixed(1)}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            {(course.instructor.instructorProfile.website || course.instructor.instructorProfile.linkedin_url || course.instructor.instructorProfile.twitter_url) && (
                                                <div className="mt-6 flex items-center gap-3">
                                                    {course.instructor.instructorProfile.website && (
                                                        <a href={course.instructor.instructorProfile.website} target="_blank" rel="noopener" className="inline-flex items-center gap-2 px-3 py-1.5 text-sm bg-slate-900/50 border border-slate-700/50 rounded-md text-slate-300 hover:text-white hover:border-slate-600">
                                                            <Globe size={16} />
                                                            Website
                                                        </a>
                                                    )}
                                                    {course.instructor.instructorProfile.linkedin_url && (
                                                        <a href={course.instructor.instructorProfile.linkedin_url} target="_blank" rel="noopener" className="inline-flex items-center gap-2 px-3 py-1.5 text-sm bg-slate-900/50 border border-slate-700/50 rounded-md text-slate-300 hover:text-white hover:border-slate-600">
                                                            <Linkedin size={16} />
                                                            LinkedIn
                                                        </a>
                                                    )}
                                                    {course.instructor.instructorProfile.twitter_url && (
                                                        <a href={course.instructor.instructorProfile.twitter_url} target="_blank" rel="noopener" className="inline-flex items-center gap-2 px-3 py-1.5 text-sm bg-slate-900/50 border border-slate-700/50 rounded-md text-slate-300 hover:text-white hover:border-slate-600">
                                                            <Twitter size={16} />
                                                            Twitter
                                                        </a>
                                                    )}
                                    ...
                                    {!course.instructor.instructorProfile && (
                                        <div className="text-slate-300">
                                            <p className="mb-4">This instructor hasn’t added a public profile yet. You can still explore their courses and reviews.</p>
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4">
                                                    <p className="text-sm text-slate-400 mb-1">Course Rating</p>
                                                    <div className="flex items-center gap-2">
                                                        <Star size={18} className="fill-yellow-400 text-yellow-400" />
                                                        <p className="text-2xl font-bold text-white">{parseFloat(course.rating || 0).toFixed(1)}</p>
                                                    </div>
                                                </div>
                                                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4">
                                                    <p className="text-sm text-slate-400 mb-1">Students</p>
                                                    <p className="text-2xl font-bold text-white">{course.total_students?.toLocaleString?.() || course.total_students || 0}</p>
                                                </div>
                                                <div className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-4">
                                                    <p className="text-sm text-slate-400 mb-1">Language</p>
                                                    <p className="text-2xl font-bold text-white capitalize">{course.language || 'en'}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                        ...
                        <section className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
                            ...
                            {isEnrolled && (
                                <form onSubmit={handleReviewSubmit} className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-6 mb-8">
                                    <h3 className="font-bold text-white text-lg mb-6">{userReview ? 'Update Your Review' : 'Share Your Experience'}</h3>

                                    <div className="mb-6">
                                        <label className="block text-slate-300 mb-3 font-semibold">Your Rating</label>
                                        <div className="flex gap-2">
                                            {[1, 2, 3, 4, 5].map(star => (
                                                ...
                                                >
                                                    <Star
                                                        size={36}
                                                        className={star <= (hoveredRating || reviewForm.rating) ? "fill-yellow-400 text-yellow-400" : "text-slate-600 hover:text-slate-500"}
                                                    />
                                                </button>
                                            ))}
                                        </div>
                            ...
                            {reviews.data.length > 0 ? (
                                <div className="space-y-4">
                                    {reviews.data.map(review => (
                                        <div key={review.id} className="bg-slate-900/50 border border-slate-700/50 rounded-xl p-6 hover:border-slate-600/50 transition-colors">
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-slate-400">{review.user.name}</p>
                                                    </div>
                                                </div>
                                                <div className="flex gap-1">
                                                    {[...Array(5)].map((_, i) => (
                                                        <Star key={i} size={16} className={i < review.rating ? "fill-yellow-400 text-yellow-400" : "text-slate-700"} />
                                                    ))}
                                                </div>
                                            </div>
                                            <p className="text-slate-300 leading-relaxed">{review.content}</p>
                                            <p className="text-xs text-slate-500 mt-3">{new Date(review.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-slate-400 text-lg">No reviews yet. Be the first to share your thoughts!</p>
                                </div>
                            )}
                        </section>
                    </div>
                            ...
                            <div className="bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 border-2 border-slate-700/50 rounded-2xl p-8 shadow-2xl backdrop-blur-sm">
                                    ...
                                    <div className="flex items-baseline gap-2">
                                        ...
                                        {course.original_price && (
                                            <span className="text-xl text-slate-500 line-through">
                                                ${parseFloat(course.original_price).toFixed(2)}
                                            </span>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 border-t border-slate-700/50 pt-6">
                                    <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg">
                                        ...
                                        <span className="text-white font-semibold capitalize px-3 py-1 bg-purple-500/20 rounded-full text-sm">{course.level}</span>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg">
                                        ...
                                        <span className="text-white font-semibold">{course.total_duration || 0} min</span>
                                    </div>
                                    
                                    <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg">
                                        <div className="flex items-center gap-3">
                                            <BookOpen className="text-green-400" size={20} />
                                            <span className="text-sm text-slate-300 font-medium">Chapters</span>
                                        </div>
                                        <span className="text-white font-semibold">{course.root_chapters?.length || 0}</span>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg">
                                        ...
                                        <span className="text-white font-semibold capitalize">{course.language}</span>
                                    </div>
                                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                    ...
                    <div className="relative w-full max-w-5xl max-h-[90vh] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 bg-slate-800/80">
                            <div>
                                <p className="text-xs text-slate-400">Chapter {selectedChapter.full_number}</p>
                                <h3 className="text-xl font-bold text-white">{selectedChapter.title}</h3>
                            </div>
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => openChapter(selectedChapter.id, 'window')}
                        ...
                        <div className="overflow-y-auto max-h-[72vh] px-6 py-5 space-y-6">
                            <div className="space-y-3">
                                ...
                                <div 
                                    ...
                                    dangerouslySetInnerHTML={{ 
                                        __html: selectedChapter.content || '<p class="text-slate-400 italic">No content provided for this chapter yet.</p>' 
                                    }}
                                />
                            </div>
                            ...
                            {selectedChapter.code_snippets?.length > 0 && (
                                <div className="space-y-3">
                                    <div className="flex items-center gap-2 text-slate-300 font-semibold">
                                        <Code size={18} className="text-emerald-300" />
                                        <span>Code Snippets</span>
                                    </div>
                                    <div className="space-y-3">
                                        {selectedChapter.code_snippets.map((snippet) => (
                                            <CodeSnippet key={snippet.id} snippet={snippet} />
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-700 bg-slate-800/80">
                            ...
                            >
                                <ArrowLeft size={16} />
                                {previousChapter ? previousChapter.title : 'No previous'}
                            </button>
                            <div className="text-slate-400 text-xs">
                                Reading {currentChapterIndex + 1} of {flatChapters.length}
                            </div>
                            <button
